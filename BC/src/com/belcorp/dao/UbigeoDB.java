...
public class UbigeoDB {
    private static final String metodoWeb = "getDivPolitica"; //BBWS7ObtenerMeta?PIN=&IMEI=&IMSI=&IDAPP=&FFVV=FS&CodigoPais=21&NombreUsuario=1215&GMT=&Rol=
    private static String URL, DATA;
    private static PersistentObject persist;
    private static final long IDSTORE = 0xefed7103f331902dL; // com.belcorp.entidades.Ubigeo
    ...
    public int getIndexById(String id) {
        ...
        if ( id != null ) {
            for (i = 0; i < n; i++) {
            	item = (Ubigeo) objetos.elementAt(i);
                if(id.trim().equals(item.getDescripcion().trim())){
                    return i;
                }
            }
        }
    ...
    public boolean saveObject(Ubigeo item) {
    	objetos.addElement(item);
    	persist.commit();
    	return true;
    }
